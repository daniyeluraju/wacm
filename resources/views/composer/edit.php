<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
    <div>
        <h2 style="font-size: 1.5rem; font-weight: 700; font-family: var(--font-heading); margin-bottom: 0.25rem;">Edit Message Draft: <?= e($draft->title) ?></h2>
        <p style="color: var(--text-secondary); font-size: 0.9rem;">Update content, customize placeholders, or replace attached media.</p>
    </div>
    <div>
...
<div class="grid-cols-2">
    ...
    <div class="card">
        ...
        <form method="POST" action="/composer/<?= e($draft->id) ?>" enctype="multipart/form-data">
            <?= csrf_field() ?>
            ...
            <div style="margin-bottom: 1.25rem;">
                ...
                <input type="text" name="title" value="<?= e(old('title', $draft->title)) ?>" required style="width: 100%; padding: 0.75rem 1rem; background-color: var(--bg-input); border: 1px solid var(--border-medium); border-radius: var(--radius-md); color: var(--text-primary); font-size: 0.9rem; outline: none;">
            </div>
            ...
            <div style="margin-bottom: 1.25rem;">
                ...
                <textarea id="messageContent" name="content" rows="8" required style="width: 100%; padding: 0.85rem 1rem; background-color: var(--bg-input); border: 1px solid var(--border-medium); border-radius: var(--radius-md); color: var(--text-primary); font-size: 0.9rem; outline: none; line-height: 1.5; resize: vertical;"><?= e(old('content', $draft->content)) ?></textarea>
            </div>
            ...
            <div style="margin-bottom: 1.5rem; padding: 1.25rem; background-color: var(--bg-surface-elevated); border: 1px solid var(--border-subtle); border-radius: var(--radius-md);">
                <label style="display: block; font-size: 0.8rem; font-weight: 600; color: var(--text-secondary); margin-bottom: 0.35rem;">Replace Image Attachment</label>
                <?php if (!empty($attachment)): ?>
                    <div style="font-size: 0.8rem; color: var(--accent-wa); margin-bottom: 0.5rem;">
                        &#10003; Current file: <?= e($attachment->original_name) ?> (<?= format_bytes((int)$attachment->file_size) ?>, <?= e($attachment->dimensions) ?>)
                    </div>
                <?php endif; ?>
    ...
    <div class="card">
        ...
        <div style="background: #0b141a; border-radius: var(--radius-lg); padding: 1.5rem; min-height: 280px; display: flex; flex-direction: column; justify-content: flex-end;">
            <div id="previewBubbleContainer" style="background-color: #005c4b; color: #e9edef; padding: 0.5rem; border-radius: 8px 8px 0px 8px; max-width: 90%; align-self: flex-end; box-shadow: 0 1px 0.5px rgba(11,20,26,.13); font-size: 0.9rem;">
                <div id="previewImageWrapper" style="<?= !empty($attachment) ? 'display: block;' : 'display: none;' ?> margin-bottom: 0.5rem; border-radius: 6px; overflow: hidden; max-height: 260px; background: rgba(0,0,0,0.2);">
                    <img id="previewImage" src="<?= !empty($attachment) ? '/attachments/' . (int)$attachment->id : '' ?>" alt="Attached image preview" style="width: 100%; height: auto; max-height: 260px; object-fit: contain; display: block; margin: 0 auto; border-radius: 6px;">
                </div>
                <div style="padding: 0.25rem 0.5rem; white-space: pre-wrap; word-break: break-word; line-height: 1.45;" id="previewBubble"></div>
            </div>
